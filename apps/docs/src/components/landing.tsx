...
function StatusPill({ status }: { status: string }) {
  return (
    <span
      className={`rounded-full px-2 py-0.5 font-mono text-[11px] ${STATUS_STYLES[status] ?? ""}`}
    >
      {status}
    </span>
  );
...
function SectionHeading({
  ...
  return (
    <div className="mx-auto max-w-2xl text-center">
      <p className="mb-3 font-mono text-fd-muted-foreground text-xs uppercase tracking-widest">
        {label}
      </p>
    ...
    <div className="py-10">
      <div className="flex items-start justify-between gap-4">
        ...
        <Link
          to="/docs/$"
          params={{ _splat: "" }}
          className="shrink-0 text-fd-muted-foreground text-sm transition-colors hover:text-fd-foreground"
        >
          Read the docs →
        </Link>
      </div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2 md:items-stretch">
        ...
        <div className="flex items-center rounded-xl border border-fd-border bg-fd-card p-5">
          {demo}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="flex w-full flex-col gap-2">
      {rows.map((row) => (
        <div
          key={row.name}
          className="flex items-center justify-between rounded-lg border border-fd-border bg-fd-background px-3 py-2 text-sm"
        >
  ...
  return (
    <section className="flex flex-col items-center px-4 pt-24 pb-16 text-center">
      <Link
        to="/docs/$"
        params={{ _splat: "" }}
        className="mb-8 inline-flex items-center gap-2 rounded-full border border-fd-border px-3 py-1 font-mono text-fd-muted-foreground text-xs"
      >
    ...
    <section className="mx-auto max-w-3xl px-4 py-16">
      <SectionHeading
        ...
        title={
          <>
            The exact same code.{" "}
            <span className="font-serif-accent text-fd-primary">Any provider.</span>
          </>
        }
...
function Features() {
  return (
    <section className="mx-auto max-w-4xl px-4 py-16">
      <SectionHeading
        label="Capabilities"
        title="Everything you need for video."
  ...
  return (
    <section className="mx-auto max-w-3xl px-4 py-16">
      <SectionHeading
        ...
        title="One interface, honest about the differences."
        ...
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr className="border-fd-border border-b">
              <th className="py-3 pr-4 text-left font-medium text-fd-muted-foreground">
                Capability
              </th>
              {columns.map((column) => (
              ...
              <tr key={row.name} className="border-fd-border/60 border-b">
                ...
                {columns.map((column) => (
                  <td key={column.id} className="px-4 py-3 text-center">
                    {row[column.id] ? (
                      <Check className="mx-auto size-4 text-fd-primary" />
                    ) : (
                      <span className="text-fd-muted-foreground">—</span>
                    )}
                  </td>
...
function GettingStarted() {
  return (
    <section className="mx-auto max-w-3xl px-4 py-16">
      <SectionHeading
        label="Two steps"
        title="Your first upload in under a minute."
        description="Install videos-sdk and one adapter, then construct a client and start calling it."
      />
